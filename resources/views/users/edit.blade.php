...
@section('content')
    <form action="{{ route(str($resource) . '.update', [Str::singular($resource) => $primary]) }}" method="post"
        ...
        <div class="mt-3">
            <label>Avatar <span class="text-secondary">(opsional)</span></label>
        ...
        <div class="form-floating mt-3">
            <input name="phone_number" type="text" id="phoneNumberTextInput" class="form-control" placeholder=""
                value="{{ old('phone_number') ?? $primary->phone_number }}" autofocus>
            <label for="nameTextInput">No. Handphone</label>
        </div>
        <div class="form-floating mt-3">
            <input name="address" type="text" id="addressTextInput" class="form-control" placeholder=""
                value="{{ old('address') ?? $primary->address }}" autofocus>
            <label for="addressnameTextInput">Alamat</label>
        </div>
        <div class="form-floating mt-3">
            <select name="level_id" class="form-select" id="levelSelectInput">
                @foreach ($secondary as $option)
                    <option value="{{ $option->id }}" @if ($primary->level->id == $option->id) selected @endif>
                        {{ $option->name }}</option>
                @endforeach
            </select>
            <label for="levelSelectInput">Level</label>
        </div>
        <div class="form-floating mt-3">
        ...
        <div class="form-floating mt-3">
            ...
            <label for="passwordPasswordInput">Password <span class="text-secondary">(opsional)</span></label>
        </div>
        ...
        <div class="row mt-2">
            <label>Di-<i>suspend</i> hingga <span class="text-secondary">(opsional)</span></label>
